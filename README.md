...
# Running
One major GAUR objective is to provide what we call "semantic information": information about the impact of the input on the application. In this proof of concept, we infer this impact by analyzing the lexical field used in the grammar file: nonterminal and terminal names, functions called, and variable names.  As we mostly focus on DBMS for now, we want to associate an action label and object label to each parser input. Directly inferring the action and object labels given the input is complex: alternate encoding and syntactic mutation prevent the usage of NLP techniques directly on the input. Furthermore integrating a NLP inference module within the application would add a significant overhead.
