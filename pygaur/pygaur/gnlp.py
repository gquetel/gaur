...
def get_tags_keywords_embeddings(
...
def get_df_tags_keywords(filepath: str) -> pd.DataFrame:
    ...
    Args:
        filepath (str): Filepath to tag file

    Returns:
        pd.DataFrame: DataFrame with columns [tag, keyword]
    """
    ltags = []
    lkeywords = []
...
def post_process_weight(df_pred: pd.DataFrame):
    ...
    """
    df_pred["modify"] = df_pred.apply(
        lambda row: max(row["create"], row["delete"], row["modify"]), axis=1
    )
...
def apply_treshold_nterm(
    df_pred: pd.DataFrame, tresholds: list, possible_tags: list
):
    ...
    for itag in range(len(possible_tags)):
        ...
        def filter_nterm(x):
            return 0 if tresholds[itag] > x else 1

        df_pred[possible_tags[itag]] = df_pred[possible_tags[itag]].apply(
            filter_nterm
        )


def compute_nterm_semantic_max(
    filepath: str,
    model,
    possible_tags: list,
    set_stop_words: set,
    ...
    for lhs, row in df_extract.iterrows():
        ...
        rhs = " ".join(
            [
                word
                for word in rhs.split()  # Stop words removal
        ...
        if row["code"]:
            try:
                ...
                for keyword in list_keywords.index:
                    tag, vector = list_keywords.loc[keyword]
                    cs = cosine_similarity(embedding_rhs, vector)
                    if cs > best_values[tag]:
                        best_values[tag] = cs

                    cs = cosine_similarity(embedding_lhs, vector)
                    if cs > best_values[tag]:
                        best_values[tag] = cs
...
def compute_nterm_semantic_mean(
    filepath: str,
    model: SentenceTransformer,
    possible_tags,
    set_stop_words,
    ...
    for lhs, row in df_extract.iterrows():
        ...
        if row["code"]:
            ...
            try:
                ...
                for keyword in list_keywords.index:
                    tag, vector = list_keywords.loc[keyword]
                    cs1 = cosine_similarity(embedding_rhs, vector[0])

                    cs2 = cosine_similarity(embedding_lhs, vector[0])
                    cs = cs1 + cs2 / 2
