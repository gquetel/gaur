 ...
 * @rule_stmt: Statement tag associated with the node.
...
#define GET_STMT_TAG(i) (ggrulesem[i - 2])
...
#define GAUR_REDUCE(nrule, yylen, yykind)                                  \
    ...
    { /* We substract 1 to nrule because of the bison accept rule offset*/ \
        n_nonterminal++;                                                   \
        reduce(yylen, nrule - 1, GET_STMT_TAG(nrule), yykind);             \
    } while (0)
 ...
 * @param r_stmt Rule stmt tag
 * @param yykind Kind of the node (given by flex / bison)
 */
void reduce(int n_children, int r_id, int r_stmt, int yykind)
{
 ...
 *  | appearance_int:symbol_kind:rule_id:stmt::sem_value
 * @param root_node
 * @param f
 */
void print_nodes_attr(node_t *root_node, FILE *f)
{
    ...
    /* Now print statement */
    for (size_t i = 0; i < sizeof(_stmts_mapping) / sizeof(_stmts_mapping[0]); i++)
    {
        if (root_node->rule_stmt & _stmts_mapping[i].value)
        {
            fprintf(f, "%s", _stmts_mapping[i].name);
            break;
        }
    }
    fprintf(f, "::");
